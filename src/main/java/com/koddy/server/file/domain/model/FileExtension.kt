...
enum class FileExtension(
    ...
    companion object {
        fun from(fileName: String): FileExtension = entries.firstOrNull { it.value == extractFileExtension(fileName) } ?: throw FileException(INVALID_FILE_EXTENSION)

        fun isImage(fileName: String): Boolean = Stream.of(JPG, JPEG, PNG).anyMatch { it.value == extractFileExtension(fileName) }
