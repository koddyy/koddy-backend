...
public class AuthenticationMentorUnivAcceptanceTest extends AcceptanceTest {
    ...
    class AuthWithMail {
        ...
        @Test
        @DisplayName("짧은 시간동안 3회 이상 인증 시도를 하게 되면 5분동안 메일인증 밴을 당한다 (HTTP Status 429)")
    ...
    class ConfirmMailAuthCode {
        ...
        void throwExceptionByInvalidAuthCode() {
            final String accessToken = MENTOR_1.회원가입_로그인_후_AccessToken을_추출한다();
            멘토가_메일을_통해서_학교_인증을_시도한다(accessToken);
            멘토가_학교_메일로_발송된_인증번호를_제출한다(AUTH_CODE + "7", accessToken)
        ...
        void success() {
            final String accessToken = MENTOR_1.회원가입_로그인_후_AccessToken을_추출한다();
            멘토가_메일을_통해서_학교_인증을_시도한다(accessToken);
            멘토가_학교_메일로_발송된_인증번호를_제출한다(AUTH_CODE, accessToken)
                    .statusCode(NO_CONTENT.value());
        }
    }
